...
void ArmController::CLIK(const Vector12d & target_x, double duration)
{
	...
	Vector3d x_cubic;
	Affine3d transform_old = model_interface_.getTransform(franka::Frame::kEndEffector, q_desired_);
	Matrix<double, 6, 7> j_old = model_interface_.getJacobianMatrix(franka::Frame::kEndEffector, q_desired_);
	Vector3d x_old = transform_old.translation();
	Matrix3d rotation_old = transform_old.linear();
	...
	x_error.segment<3>(0) = x_cubic - x_old;
	//x_error.segment<3>(3) = -0.5 * DyrosMath::getPhi(rotation_, rotation_cubic);
	x_error.segment<3>(3) = -0.5 * DyrosMath::getPhi(rotation_old, rotation_cubic);
	...
	//Vector7d qd_desired = j_old.transpose() * (j_old*j_old.transpose()).inverse() * xd_desired;

	Vector7d qd_desired = j_old.transpose() * (j_old*j_old.transpose()).inverse() * (xd_desired + kp * x_error);
